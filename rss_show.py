...
showAbbr = 'SomeShow' # filename of show without date (do not include the dash!). EG NYT
url = 'http://somesite.org/somefeed.xml'  # source RSS feed
...
def removeYesterdayFiles():
    ...
    while numberOfDestinations >= 0:
        globbify = glob.glob(
            f'{destinations[numberOfDestinations]}\{showAbbr}*.wav')
        globbifyLength = len(globbify) - 1
        if globbifyLength >= 0:
            syslog(message=f'{show}: Deleting {globbify[globbifyLength]}')
            os.remove(f'{globbify[globbifyLength]}')
        else:
            syslog(message="Cannot find yesterday's files. Continuing...")
        numberOfDestinations = numberOfDestinations - 1
...
def syslog(message):
    ...
    # don't forget this after you send the message!
...
def check_file_transferred(fileToCheck):
    ...
    try:
        ...
        while numberOfDestinations >= 0:
            ...
            numberOfDestinations = numberOfDestinations-1
            syslog(
                message=f'{show}: {fileToCheck} arrived at {destinations[numberOfDestinations]}')
        countdown()
    except:
        toSend = (f"There was a problem with {show}.\n\n\
...
def check_length(fileToCheck):
    ...
    if duration > check_if_above:
        toSend = (f"Today's {show} is {duration} minutes long! \
Please check manually and make edits to bring it below {check_if_above} minutes.\n\n\
{timestamp}")
        notify(message=toSend, subject='Check Length')
    elif duration < check_if_below:
        toSend = (f"Today's {show} is only {duration} minutes long! \
